...
class Tree():
    ...
    def _detach_node(self, node):
        ...
        if node._left_child and node._right_child:
            raise ValueError("Impossible to detach node with two children")
        if node.data == self._root_node.data:
            self._root_node = None
            del node
            return
        ...
        if parent._left_child:
            if parent._left_child.data == node.data:
                left_side = True
        ...
        if node._right_child:
            replacement = node._right_child
        else:
            replacement = node._left_child
        replacement._parent = parent
        if left_side:
            parent._left_child = replacement
        else:
            parent._right_child = replacement
        del node
